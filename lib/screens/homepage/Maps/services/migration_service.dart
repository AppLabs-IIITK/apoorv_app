...
class MigrationService {
  ...
  static Future<MigrationResult> migrateAll() async {
    ...
    await _uploadFetchedDataJsonToSupabase(
  ...
  // Uploads fetched source data to Supabase Storage for auditing/debugging.
  static Future<void> _uploadFetchedDataJsonToSupabase({
    required List<dynamic> locationsRaw,
    required List<dynamic> eventsRaw,
  }) async {
    try {
      const bucket = 'assets';
      final timestamp = DateTime.now().toUtc().toIso8601String().replaceAll(':', '-');
      final objectPath = 'migration_dumps/migration_dump_$timestamp.json';

      final payload = {
        'generated_at': DateTime.now().toUtc().toIso8601String(),
        'locations_count': locationsRaw.length,
        'events_count': eventsRaw.length,
      ...
      debugPrint('MigrationService: Uploaded source dump to $bucket/$objectPath');
      debugPrint('MigrationService: Source dump public URL: $publicUrl');

      try {
        final signedUrl =
            await _supabase.storage.from(bucket).createSignedUrl(objectPath, 604800);
        debugPrint('MigrationService: Source dump signed URL (7 days): $signedUrl');
      } catch (_) {
        // Ignore signed URL errors for public buckets / restricted policies.
      }

    } catch (e) {
      debugPrint('MigrationService: Failed to upload source dump JSON - $e');
    }
  }
